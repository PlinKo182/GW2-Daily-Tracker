...
const EventCard = ({ event, isCompleted, onToggle, itemPrices, currentTime }) => {
  ...
  return (
    <div className={`bg-gray-800 rounded-xl shadow-lg border-2 transition-all duration-300 hover:shadow-xl ${
      isCompleted ? 'border-emerald-500 bg-emerald-900/20' : 
      isExpired ? 'border-red-500 bg-red-900/20' : 
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between bg-gray-900/50 rounded-lg p-3">
            ...
            <div className="text-right">
              <span className="text-emerald-300 font-medium block">{event.reward}</span>
              {itemPrices[event.reward] && (
                <span className="text-gray-400 text-xs">({itemPrices[event.reward]} gems)</span>
              )}
            </div>
          </div>
