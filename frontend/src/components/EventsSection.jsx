...
// Separate component for event status and countdown to prevent hover flicker
const EventCardStatus = React.memo(({ 
  event, 
  currentTime, 
  completedEvents, 
  completedEventTypes, 
...
// Memoized card component with hover effects
const EventCard = React.memo(({ event, ...props }) => (
  <div className={`bg-gray-800 rounded-xl overflow-hidden shadow-lg border border-gray-700 flex flex-col relative hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${props.completedEvents[event.id] || props.completedEventTypes[event.eventKey] ? 'opacity-70 border-l-4 border-l-emerald-400' : ''}`}>
    <EventCardStatus event={event} {...props} />
  </div>
));
...
const EventsSection = ({ completedEvents, completedEventTypes, onEventToggle, currentTime }) => {
  ...
  const processEventsData = () => {
    ...
    Object.entries(mockData.eventConfig.events).forEach(([key, event]) => {
      if (key === "lla" && completedEventTypes["lla"]) return;
